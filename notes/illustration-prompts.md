# 挿絵プロンプト集｜連載「ピアノの先生になるまで」全3回

画像生成AI（ChatGPT／Gemini／Midjourney など）に貼り付けて使うプロンプト集です。
3回とも同じ「共通スタイル」を先頭につけると、シリーズとして絵柄がそろいます。

---

## 0. 共通スタイル（毎回この文を先頭にコピペ）

```
やわらかい水彩と色鉛筆で描いた、あたたかい日本の絵本のような挿絵。
淡いクリーム色の紙の質感、やさしい自然光、彩度を抑えたくすんだ色づかい。
輪郭線は細く、余白を多めに。人物の顔は描き込まず、後ろ姿・横顔・手元で表現する。
色は クリーム(#faf3ec)、くすんだローズ(#b97b74)、真鍮色(#93702f)、濃紺(#262e40) を基調に。
画面には文字・ロゴ・数字を一切入れないこと。写真ではなくイラストで。
```

**注意点**

- 「文字を入れない」は必ず毎回指定してください（AIは日本語の文字が崩れます）
- 手の指や楽譜の音符は崩れやすいので、**手元アップより引きの構図**が安全です
- 実在の先生・ご家族に似せる必要はありません。象徴的な後ろ姿で十分です
- noteのヘッダー画像は **横長（1280×670px前後・アスペクト比 16:9）** が収まりよく表示されます

---

## 第1回「4歳、兄のうしろ姿を追いかけて」

### ヘッダー画像（横長 16:9）

```
（共通スタイル）＋
昭和の日本の幼稚園の保育室。夕方の斜めの光が窓から差し込んでいる。
部屋の奥で年上の男の子がオルガンに向かって座り、
その後ろで、4歳くらいの小さな女の子が順番を待ちながら背中をじっと見ている。
女の子は後ろ姿。床には小さな椅子が並び、空気にほこりの光の粒が舞っている。
静かで、あこがれに満ちた雰囲気。横長の構図。
```

### 挿絵A｜まっさらなテキストの心残り

```
（共通スタイル）＋
古い日本の家の居間。木の机の上に、まだ一度も開かれていない新しい楽譜の本が置かれている。
本は閉じたまま、傍らに鉛筆が一本。窓からのやわらかい光が本を照らしている。
人物は登場しない、静物のような構図。少しさみしさのある、でもあたたかい雰囲気。
```

### 挿絵B｜小学1年生の夏、引っ越し

```
（共通スタイル）＋
夏の日本の家。積み上げられた引っ越しの段ボール箱の前に、
小学1年生くらいの女の子が楽譜の入ったレッスンバッグを両手で抱えて立っている。後ろ姿。
開いた窓の外には夏の入道雲。セミの声が聞こえてきそうな明るい光。
```

---

## 第2回「15歳、憧れの先生に出会う」

### ヘッダー画像（横長 16:9）

```
（共通スタイル）＋
発表会のホールの舞台袖。中学生くらいの女の子が、袖のカーテンの陰から舞台のほうを見ている。後ろ姿。
舞台の上には、憧れの女性の先生の姿がやわらかい光の中に小さく見える。
手が届かない距離感と、あこがれのまなざしが伝わる構図。横長。
```

### 挿絵A｜グループレッスンから、個人レッスンへ

```
（共通スタイル）＋
1枚の絵を左右に分けた構図。
左側：小さなオルガンが何台も並び、子どもたちが横一列に座っているグループレッスンの教室。
右側：一台のピアノの前に、女の子と先生がふたりだけで並んで座っている静かな部屋。
どちらもやさしい光。人物はすべて後ろ姿。
```

### 挿絵B｜エレクトーンの世界へ

```
（共通スタイル）＋
薄暗い部屋の中で、エレクトーン（電子オルガン）だけがやわらかく光っている。
二段の鍵盤と足鍵盤、たくさんの小さなボタンが並ぶ楽器を、少し斜め上から引きで描く。
奥に女の子が座っているが、シルエットのみ。音が空気にひろがっていくような雰囲気。
```

---

## 第3回「27年間、習い続けられた理由」

### ヘッダー画像（横長 16:9）

```
（共通スタイル）＋
ひとつの鍵盤の上に、小さな子どもの手と、大人の女性の手が並んで置かれている。
指は軽く鍵盤に触れているだけで、細部は描き込みすぎない。
27年という時間が重なっているような、静かであたたかい構図。横長、背景は淡くぼかす。
```

### 挿絵A｜アメリカ・シンシナティの音遊び

```
（共通スタイル）＋
アメリカの家のリビング。ラグの上に赤ちゃんが数人すわり、
タンバリンや鈴などの小さな楽器のおもちゃで遊んでいる。
そばに若いお母さんがひとり、床にすわって手拍子をしている。後ろ姿。
窓の外にはアメリカ郊外の家並みと大きな木。あかるい午前中の光。
```

### 挿絵B｜家族への感謝

```
（共通スタイル）＋
4人の家族の後ろ姿のシルエット。父、母、兄、そして小さな女の子。
彼らの視線の先に、光に包まれた一台のオルガンが小さく置かれている。
言葉のない、静かな感謝の場面。背景は淡いクリーム色のグラデーション。
```

### 挿絵C｜だから今、思うこと

```
（共通スタイル）＋
現在のピアノのレッスン室。グランドピアノの前に小さな生徒がひとり座り、
その斜め後ろで女性の先生が微笑みながら見守っている。ふたりとも後ろ姿。
窓からやわらかい午後の光。譜面台の上の楽譜は白いままで、文字も音符も描かない。
```

---

## 英語版（Midjourney など英語プロンプトのAI用）

### 共通スタイル（Style prefix）

```
soft watercolor and colored pencil children's book illustration, warm Japanese picture book style,
cream paper texture, gentle natural light, muted desaturated palette of cream, dusty rose, brass gold and deep navy,
thin delicate linework, generous negative space, faces not drawn in detail (back views, profiles, hands),
no text, no letters, no logos, no numbers --ar 16:9
```

### 各回のシーン（上の Style prefix のあとに続けて使用）

| 回 | シーン英文 |
| --- | --- |
| 1・ヘッダー | a little girl seen from behind, waiting and watching her older brother play a small organ in a 1970s Japanese kindergarten room, late afternoon light through the windows, dust motes in the air |
| 1・挿絵A | a brand-new unopened music book resting on a wooden desk in an old Japanese living room, a single pencil beside it, soft window light, no people, quiet and slightly wistful |
| 1・挿絵B | a first-grade girl seen from behind holding a lesson bag of sheet music, standing among stacked moving boxes, summer clouds outside the open window |
| 2・ヘッダー | a teenage girl seen from behind, peeking from the wings of a recital hall stage, a woman piano teacher standing far away in soft stage light |
| 2・挿絵A | a split composition: on the left a group music class with rows of small organs and children, on the right one piano with a single girl and her teacher sitting together, all figures seen from behind |
| 2・挿絵B | an electone (two-manual electronic organ with foot pedals) glowing softly in a dim room, seen from a slight high angle, a girl's silhouette seated at it |
| 3・ヘッダー | a child's hand and an adult woman's hand resting side by side on the same piano keys, close but softly blurred background, quiet and warm |
| 3・挿絵A | babies playing with tambourines and bells on a rug in an American living room, a young mother sitting on the floor clapping, suburban houses and a big tree outside the window, bright morning light |
| 3・挿絵B | silhouettes of a family of four seen from behind — father, mother, older brother and a little girl — looking toward a small organ bathed in light, cream gradient background |
| 3・挿絵C | a piano lesson room today, a small student at a grand piano with a woman teacher watching warmly from behind, both seen from the back, soft afternoon light, blank sheet music with no notes |

---

## 使い方のコツ

1. まず**第1回のヘッダー画像**を生成し、気に入った1枚を決めます
2. 2回目以降は「**この絵と同じタッチで**」と伝えて、その画像を添付すると絵柄がそろいます
3. できあがった画像に文字が入ってしまったら、「文字を消して」と追加で指示してください
4. noteでは、ヘッダー画像1枚＋本文中に1〜2枚あれば十分読みやすくなります
